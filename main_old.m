...
while abs(err_tot_vect(end))>1e-5
    ...
    while abs(err_rho3(end))>1e-5 || abs(err_V3(end))>1e-5
        ...
        b3 = tan(alpha3)/tan(alpha2) * b2*rho2/rho3;
    ...
    %% plot iterative process rho3

    % figure
    % semilogy(iter_rho3, err_rho3, '.-')
    % grid on
    % title('err_{\rho3}')
    % xlabel('iter')
    % ylabel('log(err_rho3)')
    % figure
    % plot(iter_rho3, rho3_vect, '.-')
    % grid on
    % title('rho3')
    % xlabel('iter')
    % ylabel('rho3')
    %
    % figure
    % semilogy(iter_rho3, err_V3, '.-')
    % grid on
    % title('err_{V3}')
    % xlabel('iter')
    % ylabel('log(err)')
    % figure
    % plot(iter_rho3, V3_vect, '.-')
    % grid on
    % title('V3')
    % xlabel('iter')
    % ylabel('V3')
    ...
    alpha4_geom = asin( (R3/R4) * sin(alpha3_geom) );
    ...
    V4_tg = R2/R4 * V2_tg *exp(-Cf_vaneless * Ld/b4/cos(alpha_mean_geom));
    V4 = V4_tg/sin(alpha4);
    V4_meridional = V4*cos(alpha4);
    dv = 2*pi*(R3*V3_tg-R4*V4_tg)/(N_bl_VD*Lb);
    L_check = dv/(V3-V4);
    t_bl_VD = 2 * Lb *tan(theta_c);
    ...
    Pt4 = Pt1 * (1+eta_int_tt*((Tt2-Tt1)/Tt2))^(gamma/(gamma-1));
...
check = {'psi', psi * 4,'[0.7-0.8] unshrouded ([0.6-0.7] shrouded)';
    ...
    'alpha2', rad2deg(alpha2), '[<80 avoiding recirculation]';
    'mer_grad', mer_grad, '[0.5-0.7]';
    'U2', U2, '[for mechanical stresses]';
    'b2/R2', b2/R2, '[stability check]';
    'AR_VD', AR_VD, '[1.4-2.4]';
    'theta_c',theta_c, '[3.5-5.5]';
    'L', L, 'loading VD [0-0.33]';
    'm. flow in',m_dot,'3';
    'm. flow 1',m_dot1,'3';
    'm. flow 2',m_dot2,'3';
    'm. flow 3',m_dot3,'3';
