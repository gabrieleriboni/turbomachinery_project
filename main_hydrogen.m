...
if ~isempty(eta_ok)
    ...
    fprintf('Portata ottimale H2:    %.4f kg/s (m_nd = %.4f)\n', m_dot_opt_H2, m_nd_opt_H2);
    fprintf('Efficienza massima H2:  %.4f\n', max_eta_H2);
    fprintf('Rapporto Compressione:  %.4f\n', beta_opt_H2);
    fprintf('==========================================\n\n');
end
...
if exist('m_nd_opt_H2', 'var')
    ...
    text(m_nd_opt_H2, beta_opt_H2, sprintf('  Peak ($\\dot{m}_{nd}=%.4f$, %.3f)', m_nd_opt_H2, beta_opt_H2), 'Interpreter', 'latex')
end

if ~isnan(m_nd_surge), xline(m_nd_surge, '--r', 'Surge (peak \beta_{tt})'); end
if ~isnan(m_nd_choke), xline(m_nd_choke, '--k', 'Choke (M \geq 1)'); end
...
if exist('m_nd_opt_H2', 'var')
    ...
    text(m_nd_opt_H2, max_eta_H2, sprintf('  Peak ($\\dot{m}_{nd}=%.4f$, %.4f)', m_nd_opt_H2, max_eta_H2), 'Interpreter', 'latex')
end
